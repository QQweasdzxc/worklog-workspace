...
function userBadge() {
  if (!session) return "";
  return `<div class="identity-badge"><span>👤 ${escapeHtml(session.name)}</span><small>${escapeHtml(session.email)}</small><button class="mini" data-logout="1">登出</button></div>`;
}
...
function sync() {
  return `<section class="panel" style="margin-top:18px"><h2>📦 同步中心</h2><p class="muted">只顯示真實狀態；未完成串接不放假按鈕。</p><div class="status"><span>Identity</span><b>${session ? "🟢 已登入" : "⚪ 未登入"}</b></div><div class="status"><span>本機資料</span><b>🟢 localStorage</b></div><div class="status"><span>Google Drive</span><b>⚪ 尚未連接</b></div><div class="status"><span>Supabase</span><b>⚪ 尚未連接</b></div></section>`;
}
