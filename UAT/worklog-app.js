...
function taskWorkspace() {
  ...
  const rows = list.length ? list.map(task => { const timeline = workJournalForTask(task); const latest = timeline[timeline.length - 1]; const latestMarkup = latest ? `<div class="task-latest-journal"><span>最新推進：</span>${escapeHtml(latest.content)} <time>（${escapeHtml(fmt(latest.createdAt))}）</time></div>` : ""; return `<article class="entry task-row ${task.status === "completed" ? "task-completed" : ""} ${taskJournalTaskId === task.id ? "task-journal-open" : ""}" data-task-card="${escapeHtml(task.id)}"><div class="task-row-core"><div class="entry-main"><div class="task-row-title">${taskPriorityBadge(task)} ${taskWorkflowBadge(task)} <b>${task.status === "completed" ? "✅" : "⬜"} ${escapeHtml(task.title)}</b></div><div class="task-progress-track" aria-label="進度 ${task.progress}%"><span style="width:${task.progress}%"></span></div><small>${task.dueDate ? `期限：${escapeHtml(task.dueDate)}` : "無期限"}｜${escapeHtml(typeof PriorityEngine !== "undefined" ? PriorityEngine.getReason(task) : "依目前工作優先順序。")} ${task.note ? `｜${escapeHtml(task.note)}` : ""}</small>${latestMarkup}${task.status === "completed" && task.completedNote ? `<div class="task-completion-note">完成說明：${escapeHtml(task.completedNote)}</div>` : ""}</div><div class="actions compact"><button class="btn2" type="button" data-task-journal="${escapeHtml(task.id)}">${taskJournalTaskId === task.id ? "收合紀錄" : "工作紀錄"}</button><button class="btn2" type="button" data-task-start="${escapeHtml(task.id)}" ${task.status === "completed" ? "disabled" : ""}>${task.startedAt ? "繼續工作" : "開始工作"}</button><button class="btn2" type="button" data-task-toggle="${escapeHtml(task.id)}">${task.status === "completed" ? "恢復" : "完成"}</button><button class="btn2" type="button" data-task-edit="${escapeHtml(task.id)}">編輯</button><button class="btn2 danger" type="button" data-task-delete="${escapeHtml(task.id)}">刪除</button></div></div>${taskJournalPanel(task)}</article>`; }).join("") : `<div class="empty"><b>${taskSearch ? "找不到符合的待辦事項" : "目前還沒有待辦事項"}</b><div class="muted">可以建立今天第一個待辦事項。</div></div>`;
  const completionTask = taskCompletionDialogId ? tasks.find(task => task.id === taskCompletionDialogId) : null;
